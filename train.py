...
if __name__ == "__main__":
    ...
    try:
        ...
        train_df['floor'] = train_df['floor'].fillna('1').apply(lambda x: float(x) if type(x) == float or x.isdecimal() else 1)
        ...
        X_offer_date = train_df[train_df.price_type == PriceTypeEnum.OFFER_PRICE]['date']
        ...
        X_manual_date = train_df[train_df.price_type == PriceTypeEnum.MANUAL_PRICE]['date']
        ...
        model.cross_validate(X_offer, X_offer_date, y_offer, X_manual, X_manual_date, y_manual)
        logger.info('Fit model')
        model.fit(X_offer, y_offer, X_manual, y_manual)
        logger.info('Save model')
        model.save(args['mp'])

        print(list(sorted(zip(X_manual.columns, model.model.feature_importances_), key=lambda x: x[1])))
