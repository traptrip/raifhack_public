...
class BenchmarkModel():
    ...
    def fit(self, X_offer: pd.DataFrame, y_offer: pd.Series,
        ...
        self.pipeline.fit(
            ...
            pd.concat([y_offer, y_manual]), 
            model__feature_name=[f'{i}' for i in range(71)], model__categorical_feature=['68','69','70']
        )
    ...
    def cross_validate(self, X_offer: pd.DataFrame, X_offer_date: pd.Series, y_offer: pd.Series,
            X_manual: pd.DataFrame, X_manual_date: pd.Series, y_manual: pd.Series):
        """
        :param X_offer: pd.DataFrame с объявлениями
        :param y_offer: pd.Series - цена предложения (в объявлениях)
        :param X_manual: pd.DataFrame с ручными оценками
        :param y_manual: pd.Series - цены ручника
        """
        logger.info('cross validate lightgbm')

        offer_month = pd.to_datetime(X_offer_date).dt.month
        manual_month = pd.to_datetime(X_manual_date).dt.month

        metrics = []
        corr_coefs = []
        for month in trange(manual_month.min(), manual_month.max() - 2):

            X_train = pd.concat([
                X_offer[offer_month < month + 3],
                X_manual[manual_month < month + 3],
            ])
            y_train = pd.concat([
                y_offer[offer_month < month + 3],
                y_manual[manual_month < month + 3],
            ])

            X_val = X_manual[(manual_month >= month + 3) & (manual_month < month + 6)]
            y_val = y_manual[(manual_month >= month + 3) & (manual_month < month + 6)]
            ...
            corr_coef = self._find_corr_coefficient(X_val, y_val)
            pred = self.pipeline.predict(X_val)
            pred = pred * (1 + corr_coef)
            metrics.append(deviation_metric(y_val.to_numpy(), pred))
            corr_coefs.append(corr_coef)


        logger.info(f'Cross validation raif_metric: {metrics}')
        logger.info(f'Corr_coefficient: {corr_coefs}')
